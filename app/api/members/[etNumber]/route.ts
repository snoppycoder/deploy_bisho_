...
// 		// Calculate total savings from transactions with type "SAVINGS"
// 		const totalSavings = member.transactions
// 			.filter((transaction) => transaction.type === "SAVINGS")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);

// 		// Calculate total contributions as the sum of SAVINGS, MEMBERSHIP_FEE, LOAN_REPAYMENT, and WILLING_DEPOSIT
// 		const contributionTypes = [
// 			"SAVINGS",
// 			"MEMBERSHIP_FEE",
// 			"LOAN_REPAYMENT",
...
// 		const totalLoanRepayment = member.transactions
// 			.filter((transaction) => transaction.type === "LOAN_REPAYMENT")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);

// 		const totalMembershipFee = member.transactions
// 			.filter((transaction) => transaction.type === "MEMBERSHIP_FEE")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);

// 		const totalWillingDeposit = member.transactions
// 			.filter((transaction) => transaction.type === "WILLING_DEPOSIT")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);

// 		const totalRegistrationFee = member.transactions
// 			.filter((transaction) => transaction.type === "REGISTRATION_FEE")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);

// 		const totalCostOfShare = member.transactions
// 			.filter((transaction) => transaction.type === "COST_OF_SHARE")
// 			.reduce((sum, transaction) => sum + Number(transaction.amount), 0);
...
// 		// Get the last savings transaction
// 		const lastSavingsTransaction = member.transactions
// 			.filter((transaction) => transaction.type === "SAVINGS")
// 			.sort(
// 				(a, b) =>
// 					new Date(b.transactionDate).getTime() -
...
// 		// Get the last contribution transaction
// 		const lastContributionTransaction = member.transactions
// 			.filter((transaction) => contributionTypes.includes(transaction.type))
// 			.sort(
// 				(a, b) =>
...
// 		// Prepare savings history for chart (from transactions)
// 		const savingsHistory = member.transactions
// 			.filter((transaction) => transaction.type === "SAVINGS")
// 			.map((transaction) => ({
// 				date: transaction.transactionDate,
// 				amount: Number(transaction.amount),
...
// 		// Prepare loan repayment history for chart
// 		const loanRepaymentHistory = member.transactions
// 			.filter((transaction) => transaction.type === "LOAN_REPAYMENT")
// 			.map((transaction) => ({
// 				date: transaction.transactionDate,
// 				amount: Number(transaction.amount),
...
// 					progress: (totalRepaid / Number(loan.amount)) * 100,
// 				};
// 			});
